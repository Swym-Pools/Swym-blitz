import { _ as _createForOfIteratorHelperLoose, P as PaginationArgumentError, A as AuthenticationError, i as AuthorizationError, p as _createClass, m as COOKIE_ANONYMOUS_SESSION_TOKEN, n as COOKIE_SESSION_TOKEN, o as COOKIE_REFRESH_TOKEN, g as HEADER_CSRF, l as SESSION_TOKEN_VERSION_0, f as CSRFTokenMismatchError, j as HANDLE_SEPARATOR, k as SESSION_TYPE_ANONYMOUS_JWT, S as SESSION_TYPE_OPAQUE_TOKEN_SIMPLE, T as TOKEN_SEPARATOR, a as COOKIE_CSRF_TOKEN, H as HEADER_PUBLIC_DATA_TOKEN, C as COOKIE_PUBLIC_DATA_TOKEN, e as HEADER_CSRF_ERROR, c as HEADER_SESSION_REVOKED, d as HEADER_SESSION_CREATED } from '../../dist/constants-52bb98df.esm.js';
import { getConfig, getProjectRoot } from '@blitzjs/config';
import { log, baseLogger, chalk } from '@blitzjs/display';
...
import '../../dist/blitz-data-484e455b.esm.js';
import 'htmlescape';
import 'react';
...
function getAllMiddlewareForModule(resolverModule) {
  ...
  if (config.middleware) {
    ...
    middleware.push.apply(middleware, config.middleware);
  }
...
async function handleRequestWithMiddleware(req, res, middleware, _temp) {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      baseLogger().error("Error occured in middleware after the response was already sent to the browser");
    } else {
...
async function invokeWithMiddleware(resolver, params, ctx) {
  ...
  middleware.push(async function (_req, res, next) {
    var log$1 = baseLogger().getChildLogger({
      prefix: [enhancedResolver._meta.name + "()"]
    });
    log.newline();

    try {
      log$1.info(chalk.dim("Starting with input:"), params);
      var startTime = Date.now();
      var result = await enhancedResolver(params, res.blitzCtx);
      var duration = Date.now() - startTime;
      log$1.info(chalk.dim("Finished in " + prettyMs(duration)));
      log.newline();
...
async function paginate(_ref) {
  ...
  if (take > maxTake) {
    throw new PaginationArgumentError("`take` argument must less than `maxTake` which is currently " + maxTake);
  }
...
function pipe() {
  ...
  return async function (input, ctx) {
    ...
    for (var _iterator = _createForOfIteratorHelperLoose(functions), _step; !(_step = _iterator()).done;) {
      var fn = _step.value;
...
var getDb = function getDb() {
  ...
  var path = join(projectRoot, ".next/blitz/db.js"); // eslint-disable-next-line no-eval -- block webpack from following this module path
...
var sessionMiddleware = function sessionMiddleware(sessionConfig) {
  ...
  global.sessionConfig = _extends({}, defaultConfig, sessionConfig); // Checks if cookie prefix from configuration has
...
function ensureBlitzApiRequest(req) {
  if (!("cookies" in req)) {
    req.cookies = getCookieParser(req)();
  }
...
var parseSessionToken = function parseSessionToken(token) {
  var _fromBase64$split = fromBase64(token).split(TOKEN_SEPARATOR),
...
var createAnonymousSessionToken = function createAnonymousSessionToken(payload) {
  ...
  return sign((_jwtSign = {}, _jwtSign[JWT_NAMESPACE] = payload, _jwtSign), getSessionSecretKey(), {
...
var parseAnonymousSessionToken = function parseAnonymousSessionToken(token) {
  ...
  try {
    var fullPayload = verify(token, secret, {
...
var setSessionCookie = function setSessionCookie(req, res, sessionToken, expiresAt) {
  setCookie(res, cookie.serialize(COOKIE_SESSION_TOKEN(), sessionToken, {
    path: "/",
    httpOnly: true,
    secure: !process.env.DISABLE_SECURE_COOKIES && process.env.NODE_ENV === "production" && !isLocalhost(req),
...
async function getSessionKernel(req, res) {
  var anonymousSessionToken = req.cookies[COOKIE_ANONYMOUS_SESSION_TOKEN()];
  var sessionToken = req.cookies[COOKIE_SESSION_TOKEN()]; // for essential method

  var idRefreshToken = req.cookies[COOKIE_REFRESH_TOKEN()]; // for advanced method

  var enableCsrfProtection = req.method !== "GET" && req.method !== "OPTIONS" && !process.env.DISABLE_CSRF_PROTECTION;
  var antiCSRFToken = req.headers[HEADER_CSRF];
  ...
  if (sessionToken) {
    ...
    if (enableCsrfProtection && persistedSession.antiCSRFToken !== antiCSRFToken) {
      if (!antiCSRFToken) {
        log.warning("This request is missing the " + HEADER_CSRF + " header. You can learn about adding this here: https://blitzjs.com/docs/session-management#manual-api-requests");
      }
...
async function createNewSession(args) {
  ...
  if (args.anonymous) {
    ...
    var newPublicData = _extends({}, ((_args$jwtPayload = args.jwtPayload) == null ? void 0 : _args$jwtPayload.publicData) || {}, args.publicData);
    ...
    var newPrivateData = _extends({}, existingPrivateData, args.privateData);
...
async function refreshSession(req, res, sessionKernel, _ref2) {
  ...
  if ((_sessionKernel$jwtPay = sessionKernel.jwtPayload) != null && _sessionKernel$jwtPay.isAnonymous) {
    var payload = _extends({}, sessionKernel.jwtPayload, {
...
async function syncPubicDataFieldsForUserIfNeeded(userId, data) {
  ...
  if (Object.keys(dataToSync).length) {
    ...
    for (var _iterator = _createForOfIteratorHelperLoose(sessions), _step; !(_step = _iterator()).done;) {
      var session = _step.value;
      var publicData = JSON.stringify(_extends({}, session.publicData ? JSON.parse(session.publicData) : {}, dataToSync));
...
async function revokeMultipleSessions(req, res, sessionHandles) {
  ...
  for (var _iterator2 = _createForOfIteratorHelperLoose(sessionHandles), _step2; !(_step2 = _iterator2()).done;) {
...
async function setPrivateData(sessionKernel, data) {
  ...
  var privateData = JSON.stringify(_extends({}, existingPrivateData, data));
  await global.sessionConfig.updateSession(sessionKernel.handle, {
    privateData: privateData
  });
}
async function setPublicData(req, res, sessionKernel, data) {
  ...
  var publicData = _extends({}, await getPublicData(sessionKernel), data);

  await refreshSession(req, res, _extends({}, sessionKernel, {
    publicData: publicData
  }), {
...
function passportAuth(config) {
  return async function authHandler(req, res) {
    ...
    var middleware = [connectMiddleware(cookieSessionMiddleware), connectMiddleware(passportMiddleware), connectMiddleware(passport.session())];
    ...
    if (req.query.auth.length === 1) {
      ...
      middleware.push(connectMiddleware(function (req, res, next) {
        var session = res.blitzCtx.session;
        assert(session, "Missing Blitz sessionMiddleware!");
        passport.authenticate(strategyName, async function (err, result) {
          try {
            var error = err;

            if (!error) {
              if (result === false) {
                log.warning("Login via " + strategyName + " failed - usually this means the user did not authenticate properly with the provider");
                error = "Login failed";
              }
  ...
  return async function (req, res, next) {
    ...
    if (req.method === "HEAD") {
      ...
      try {
        ...
        log$1.info(chalk$1.dim("Starting with input:"), data ? data : JSON.stringify(data));
        var startTime = Date.now();
        var result = await resolver(data, res.blitzCtx);
        var resolverDuration = Date.now() - startTime;
        log$1.debug(chalk$1.dim("Result:"), result ? result : JSON.stringify(result));
        ...
        log$1.debug(chalk$1.dim("Next.js serialization:" + prettyMs(Date.now() - nextSerializerStartTime)));
        var serializerDuration = Date.now() - serializerStartTime;
        var duration = Date.now() - startTime;
        log$1.info(chalk$1.dim("Finished: resolver:" + prettyMs(resolverDuration) + " serializer:" + prettyMs(serializerDuration) + " total:" + prettyMs(duration)));
        log.newline();
        return next();
      } catch (error) {
        if (error._clearStack) {
        ...
        var serializedError = serialize(error);
        res.json({
          result: null,
      ...
      log$1.warn(req.method + " method not supported");
      res.status(404).end();
      return next();
    }
  };
...
export { SecurePassword, connectMiddleware, generateToken, getAllMiddlewareForModule, getSession, handleRequestWithMiddleware, hash256, invokeWithMiddleware, isLocalhost, paginate, passportAuth, resolver, rpcApiHandler, sessionMiddleware, simpleRolesIsAuthorized };
