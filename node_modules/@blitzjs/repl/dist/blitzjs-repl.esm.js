...
var projectRoot$2 = /*#__PURE__*/getProjectRoot();
...
var projectRoot$1 = /*#__PURE__*/getProjectRoot();
async function getBlitzModulePaths() {
  var paths = await globby(["app/**/{queries,mutations}/*.{js,ts,tsx}", "utils/*.{js,ts,tsx}", "jobs/**/*.{js,ts,tsx}", "integrations/**/*.{js,ts,tsx}", "!**/*.test.*", "!**/*.spec.*"], {
    cwd: projectRoot$1,
    gitignore: true
  });
  paths.push("db");
  ...
  return [].concat(paths.map(function (p) {
    return path.join(projectRoot$1, p);
  }));
}
var loadBlitz = async function loadBlitz() {
  var paths = await getBlitzModulePaths();
  var percentage = new ProgressBar("Loading Modules :current/:total", {
    total: paths.length
  });
  var modules = Object.assign.apply(Object, [{}].concat(paths.map(function (modulePath) {
    ...
    if (name === "index") {
      var dirs = path.dirname(modulePath).split(path.sep);
...
var projectRoot = /*#__PURE__*/getProjectRoot();
...
var setupSelfRolledHistory = function setupSelfRolledHistory(repl, path) {
  function init() {
    try {
      ...
      var history = fs.readFileSync(path, {
        encoding: "utf8"
      });
      var nonEmptyLines = history.split(os.EOL).filter(function (line) {
        return line.trim();
      });
  ...
  function onExit() {
    var addedHistory = repl.lines.join(os.EOL);
...
var setupHistory = function setupHistory(repl) {
  var blitzConsoleHistoryPath = path.join(projectRoot, ".blitz-console-history");
