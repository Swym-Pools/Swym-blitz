import path, { isAbsolute, normalize, resolve, relative, join } from 'path';
...
var pipeline = /*#__PURE__*/pumpify.ctor({
  autoDestroy: false,
  destroy: false,
  objectMode: true,
  highWaterMark: 10000
...
function createDisplay() {
  ...
  var spinner = log.spinner("Compiling").start();
  var stream = through({
    objectMode: true
...
function rimraf(f, opts) {
  ...
  return new Promise(function (res, rej) {
    rimrafCallback(f, opts, function (error) {
      if (error) {
        rej(error);
      } else {
        res();
...
var watch = function watch(includePaths, options) {
  ...
  function processEvent(evt) {
    return async function (filepath, _stat) {
      ...
      var file = evt === "unlink" || evt === "unlinkDir" ? new File(fileOpts) : await vinyl.read(filepath, fileOpts);
      file.event = evt;
      stream.push(file);
    };
  }

  var fswatcher = chokidar.watch(includePaths, options);
...
function agnosticSource(_ref) {
  ...
  var vinylFsStream = vfs.src(allGlobs, {
    buffer: true,
    read: true,
    dot: true,
    cwd: cwd,
  ...
  var stream = mergeStream(vinylFsStream, watcher.stream);
...
function transform(transformFn, options) {
  ...
  return through(mergedOpts, async function (item, _, next) {
    await processInput({
...
var defaultSaveCache = /*#__PURE__*/debounce(function (filePath, data) {
  return writeFile(filePath, Buffer.from(JSON.stringify(data, null, 2))).then(function () {})["catch"](function () {});
}, 500);
...
function createWorkOptimizer(src, dest, overrideTriage, saveCache, readCache) {
  ...
  var doneCacheLocation = resolve(dest, ".blitz.incache.json");
...
writeStream, unlinkStream) {
  ...
  var stream = gulpIf(isFile, gulpIf(isUnlinkFile, pipeline(unlinkStream, splitToBus), pipeline(writeStream, splitToBus)));
  return {
    stream: stream
  };
};
...
source, writer) {
  ...
  var ignorer = through.obj(function (file, _, next) {
    if (file && file.path) {
      var match = micromatch.isMatch(file.path, config.ignore);
...
var defaultBus = /*#__PURE__*/through.obj();
/**
...
async function transformFiles(src, stages, dest, options) {
  ...
  if (process.platform === "win32") await rimraf(join(dest, ".next"), {
    glob: false
  });
  ...
  return await new Promise(function (resolve, reject) {
    ...
    pipe(bus, display.stream, function (err) {
      if (err) reject(err);
    });
  });
}

export { ERROR_THROWN, FILE_DELETED, FILE_WRITTEN, FileCache, IDLE, INIT, READY, RouteCache, rimraf, transform, transformFiles };
