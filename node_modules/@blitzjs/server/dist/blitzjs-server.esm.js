import { transformFiles, transform, ERROR_THROWN, rimraf } from '@blitzjs/file-pipeline';
import { pathExists, readFile, writeFile, readJSON, readJSONSync, existsSync as existsSync$1, pathExistsSync, outputFileSync, copy } from 'fs-extra';
import path, { resolve, join, relative } from 'path';
import fs, { promises, existsSync, readFile as readFile$1 } from 'fs';
...
import 'parallel-transform';
import pumpify from 'pumpify';
import debounce from 'lodash/debounce';
import flow from 'lodash/flow';
import { debounce as debounce$1, partition as partition$1 } from 'lodash';
...
async function isVersionMatched(buildFolder) {
  ...
  var versionStore = resolve(buildFolder, blitzVersionFilename);
  if (!(await pathExists(versionStore))) return false;

  try {
    var buffer = await readFile(versionStore);
...
function globalGitIgnore() {
  var versionResult = spawn.sync("git", ["version"]);
  ...
  var configResult = spawn.sync("git", ["config", "--get", "core.excludesfile"], {
    stdio: "pipe"
  });
...
function getAllGitIgnores(rootFolder) {
  ...
  var files = fastGlob.sync([localRepoIgnore, "**/.gitignore", "**/" + localRepoIgnore], {
    cwd: rootFolder
  });
  ...
  return files.filter(isControlledByUser).map(function (file) {
    ...
    return {
      gitIgnore: fs.readFileSync(file, {
        encoding: "utf8"
      }),
      prefix: prefix
    };
...
function chokidarRulesFromGitignore(_ref) {
  var gitIgnore = _ref.gitIgnore,
      prefix = _ref.prefix;
  var rules = parseGitignore(gitIgnore);

  var isInclusionRule = function isInclusionRule(rule) {
    return rule.startsWith("!");
  };

  var _partition = partition(rules, isInclusionRule),
...
async function normalize(config) {
  ...
  return _extends({}, config, {
    ...
    clean: config.clean,
    // -
    transformFiles: (_config$transformFile = config.transformFiles) != null ? _config$transformFile : transformFiles,
...
var absolutePathTransform = function absolutePathTransform(sourceFolder) {
  ...
  return function (relativeTransformer) {
    return function (filePath) {
      var startingPath = relative(sourceFolder, filePath);
...
var createStageRpc = function createStageRpc(isTypeScript) {
  ...
  return function configure(_ref) {
    ...
    var stream = transform.file(function (file, _ref2) {
      var next = _ref2.next,
          push = _ref2.push;
      ...
      if (["add", "unlink"].includes(file.event)) {
        push(new File({
          ...
          contents: Buffer.from(apiHandlerTemplate(originalPath, isTypeScript, existsSync("db/index." + (isTypeScript ? "ts" : "js")))),
          // Appending a new file to the output of this particular stream
          // We don't want to reprocess this file but simply add it to the output
          // of the stream here we provide a hash with some information for how
          // this file came to be here
...
function createOutputTransformer(buildFolder, manifest) {
  ...
  var stream = through(function (data, _, next) {
    ...
    if (outputStr.match(/Error:.*find.*production build/)) {
      outputStr = log.withError("Could not find a production build, you must run `blitz build` before starting\n\n");
    } else if (manifest) {
      /*
      ...
      if (pageMatches) {
        ...
        if (fullMatch) {
          var builtPath = path.join(buildFolder, simplePath);
...
async function nextStartDev(nextBin, cwd, manifest, buildFolder, config) {
  ...
  return new Promise(function (res, rej) {
    if (config.port && availablePort !== config.port) {
      ...
      var nextjs = spawn$1(nextBin, spawnCommand, {
        cwd: cwd,
        env: spawnEnv,
...
function nextBuild(nextBin, buildFolder, manifest, config) {
  var spawnEnv = getSpawnEnv(config);
  return new Promise(function (res, rej) {
    var nextjs = spawn$1(nextBin, ["build"], {
      cwd: buildFolder,
      env: spawnEnv,
      stdio: [process.stdin, "pipe", "pipe"]
    }).on("exit", function (code) {
...
function nextExport(nextBin, config) {
  var spawnEnv = getSpawnEnv(config);
  return new Promise(function (res, rej) {
    var nextjs = spawn$1(nextBin, ["export"], {
      env: spawnEnv,
      stdio: [process.stdin, "pipe", "pipe"]
    }).on("exit", function (code) {
      if (code === 0 || code === null) {
...
function getCustomServerPath() {
  ...
  if (existsSync$1(serverPath)) return serverPath;
...
function startCustomServer(cwd, config, _temp) {
  ...
  return new Promise(function (res, rej) {
    ...
    var spawnServer = function spawnServer() {
      process = spawn$1("node", [serverBuildPath], {
        cwd: cwd,
        env: spawnEnv,
        stdio: "inherit"
      }).on("exit", function (code) {
      ...
      esbuildOptions.watch = {
        onRebuild: function onRebuild(error) {
          if (error) {
            ...
            log.newline(); //@ts-ignore -- incorrect TS type from node
...
var createStageConfig = function createStageConfig(_ref) {
  ...
  // Preconditions
  var hasNextConfig = pathExistsSync(resolve(config.src, "next.config.js"));
  var hasBlitzConfig = pathExistsSync(resolve(config.src, "blitz.config.js")) || pathExistsSync(resolve(config.src, "blitz.config.ts"));
  ...
  var stream = transform.file(function (file) {
...
var createStageManifest = async function createStageManifest(writeManifestFile, buildFolder, env, manifestPath) {
  ...
  var stage = function stage() {
    var debouncePushItem = debounce(function (push, file) {
      push(file);
    }, 500);
    var stream = transform.file(function (file, _ref) {
      ...
      if (writeManifestFile) {
        debouncePushItem(push, new File({
...
var ManifestLoader = {
  load: function load(filename) {
    return new Promise(function (resolve, reject) {
      readFile$1(filename, "utf8", function (err, data) {
        if (err) {
          return reject(err);
        }
...
var renderErrorMessage = function renderErrorMessage(path, type) {
  return "- " + path.split(type + "/")[0] + type + "/" + log.variable(path.split(type + "/")[1]);
};
...
function apiPathTransformer(path) {
  var regex = /(?:[\\/]?app[\\/].*?[\\/]?)(api[\\/].+(?<!\.test))$/;
  var matchedPath = (regex.exec(path) || [])[1];
  return matchedPath ? join("pages", matchedPath) : path;
}
var fullTransformer = /*#__PURE__*/flow(pagesPathTransformer, apiPathTransformer); // Derived from
...
var isInSpecialFolderInAppFolder = function isInSpecialFolderInAppFolder(s, cwd) {
  var filepath = s.replace(cwd + path.sep, "");
...
var isInAppFolder = function isInAppFolder(s, cwd) {
  return s.replace(cwd + path.sep, "").indexOf("app") === 0;
};
/**
 * Returns a Stage that converts relative files paths to absolute
 */
...
function getImportType(absoluteOrigin, cwd) {
  var foundFiles = fastGlob.sync( // if absoluteOrigin is a file import,
...
function makeDebouncedWriter(path, ms) {
  ...
  return debounce$1(function (contents) {
...
var isPage = function isPage(path) {
  return path.includes(join("pages", "/")) && !path.includes(join("pages", "api", "/")) && !path.includes(join("/", "_app.")) && !path.includes(join("/", "_document."));
};
...
function parseParametersFromRoute(path) {
  ...
  var _partition = partition$1(withoutBrackets, function (p) {
    return p.includes("...");
  }),
...
function findNodeModulesRoot(src) {
  var nodeModules = join(src, "node_modules");
  var includesBlitzPackage = existsSync$1(join(nodeModules, "blitz"));
...
var createStageRouteImportManifest = function createStageRouteImportManifest(_ref4) {
  ...
  var routes = {};
  var dotBlitz = join(findNodeModulesRoot(config.src), ".blitz");
  var writeManifestImplementation = makeDebouncedWriter(join(dotBlitz, "index.js"));
  var writeManifestBrowserImplementation = makeDebouncedWriter(join(dotBlitz, "index-browser.js"));
  var writeManifestDeclaration = makeDebouncedWriter(join(dotBlitz, "index.d.ts"));
  var stream = transform.file(function (file) {
...
async function build(config) {
  ...
  var rootNextFolder = resolve(rootFolder, ".next");
  var buildNextFolder = resolve(buildFolder, ".next");
...
export { ManifestLoader, blitzExport, build, dev, generate, normalize, prod, resolveBinAsync, routes };
