...
var Generator = /*#__PURE__*/function (_EventEmitter) {
  ...
  _proto.process = function process(input, pathEnding, templateValues, prettierOptions) {
    ...
    if (!this.useTs && tsExtension.test(pathEnding)) {
      ...
      return ((_babel$transform = transform(templatedFile, {
...
var logFailedVersionFetch = function logFailedVersionFetch(dependency, fallback) {
  log.clearLine(log.withWarning("Failed to fetch latest version of '" + chalk.bold(dependency) + "', falling back to '" + chalk.bold(fallback) + "'.\n"));
};
  ...
  _proto.postWrite = async function postWrite() {
    ...
    if (!fallbackUsed && !this.options.skipInstall) {
      ...
      await new Promise(function (resolve) {
        ...
        var cp = spawn(_this2.options.yarn ? "yarn" : "npm", ["install", logFlag], {
      ...
      spinner.fail(chalk.red.bold("We had some trouble connecting to the network, so we'll skip installing your dependencies right now. Make sure to run " + (this.options.yarn ? "'yarn'" : "'npm install'") + " once you're connected again."));
    }
...
var Field = /*#__PURE__*/function () {
  ...
  function Field(name, options) {
    ...
    if (!options.type) {
      log.warning("No field type specified for field " + log.variable(name) + ", falling back to " + log.variable("String") + ".");
