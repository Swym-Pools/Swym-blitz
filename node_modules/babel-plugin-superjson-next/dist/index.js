...
function wrapExportDefaultDeclaration(path) {
    ...
    var node = path.node;
    if (types_1.isIdentifier(node.declaration)) {
        node.declaration = wrapInHOC(node.declaration);
    }
    if (types_1.isFunctionExpression(node.declaration)) {
        node.declaration = wrapInHOC(node.declaration);
    }
    if (types_1.isFunctionDeclaration(node.declaration) ||
    ...
    return {
        ...
        visitor: {
            Program: function (path, state) {
                ...
                body
                    .filter(function (path) { return types_1.isExportNamedDeclaration(path); })
